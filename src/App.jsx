...
import FLAG_GRID from './flagGrid.js'
...
// Dato un tentativo, calcola quali celle della bandiera target vengono rivelate:
// una cella target è rivelata se il suo nome-colore appare in QUALSIASI cella del tentativo
function getRevealedCells(targetCode, guessCode) {
  const targetGrid = FLAG_GRID[targetCode]
  const guessGrid  = FLAG_GRID[guessCode]
  if (!targetGrid || !guessGrid) return new Set()

  // Tutti i colori presenti nel tentativo
  const guessColors = new Set()
  guessGrid.forEach(row => row.forEach(cell => guessColors.add(cell.name)))

  // Celle target rivelate: quelle il cui colore è nel set del tentativo
  const revealed = new Set()
  targetGrid.forEach((row, ri) =>
    row.forEach((cell, ci) => {
      if (guessColors.has(cell.name)) revealed.add(`${ri}-${ci}`)
    })
  )
  return revealed
}

// Accumula celle rivelate da tutti i tentativi
function getAllRevealedCells(targetCode, guesses) {
  const all = new Set()
  guesses.forEach(g => {
    const cells = getRevealedCells(targetCode, g.code)
    cells.forEach(k => all.add(k))
  })
  return all
}
...
const STORAGE_PREFIX = 'flagle2_game_'
...
// ── FlagCanvas: disegna la bandiera ricostruita ───────────────────────────────

function FlagCanvas({ targetCode, revealedCells, done, size = 200 }) {
  const grid = FLAG_GRID[targetCode]
  if (!grid) return null

  const cellW = size / GRID_W
  const cellH = (size * 0.6) / GRID_H
  const totalH = cellH * GRID_H

  return (
    <div style={{ width: size, height: totalH, display: 'grid', gridTemplateColumns: `repeat(${GRID_W}, 1fr)`, gridTemplateRows: `repeat(${GRID_H}, 1fr)`, borderRadius: 6, overflow: 'hidden', border: '2px solid rgba(255,255,255,0.15)' }}>
      {grid.map((row, ri) =>
        row.map((cell, ci) => {
          const key = `${ri}-${ci}`
          ...
          return (
            <div
              key={key}
              style={{
                background: isRevealed ? cell.hex : 'transparent',
                border: isRevealed ? 'none' : '0.5px solid rgba(255,255,255,0.06)',
...
function GameBoard({ dateStr, onBack, isDark, C, isToday, countdown }) {
  ...
  const revealedCells = getAllRevealedCells(country.code, game.guesses)
  const targetGrid = FLAG_GRID[country.code] || []
  const totalCells = GRID_W * GRID_H
  const revealPct = Math.round((revealedCells.size / totalCells) * 100)

  // Celle rivelate dall'ultimo tentativo (per highlight)
  const lastRevealedCells = game.guesses.length > 0
    ? getRevealedCells(country.code, game.guesses[game.guesses.length-1].code)
    : new Set()
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', width:'100%', maxWidth:500, margin:'0 auto', padding:'0 8px' }}>
      ...
      <div style={{ animation: shake ? 'shake 0.5s' : 'none', marginBottom:8 }}>
        <div style={{ width:200, height:120, display:'grid', gridTemplateColumns:`repeat(${GRID_W},1fr)`, gridTemplateRows:`repeat(${GRID_H},1fr)`, borderRadius:6, overflow:'hidden', border:`2px solid ${C.headerBorder}`, background: isDark ? '#1a1a1a' : '#e8e8e8' }}>
          {targetGrid.map((row, ri) =>
            row.map((cell, ci) => {
              const key = `${ri}-${ci}`
              const isRevealed = revealedCells.has(key)
              const isNew = lastRevealedCells.has(key) && !game.done
              return (
                <div key={key} style={{
                  background: isRevealed ? cell.hex : 'transparent',
                  outline: isNew ? '1px solid rgba(255,255,255,0.8)' : 'none',
                  outlineOffset: '-1px',
        ...
        {/* Barra progresso */}
        <div style={{ height:3, background: isDark?'#2a2a2a':'#ddd', borderRadius:2, marginTop:4, overflow:'hidden' }}>
          <div style={{ height:'100%', width:`${revealPct}%`, background:C.accent, transition:'width 0.4s', borderRadius:2 }} />
        </div>
        <div style={{ fontSize:10, color:C.textMuted, textAlign:'center', marginTop:3 }}>
          {game.done
            ? (game.won ? `✓ ${country.name}` : `Risposta: ${country.name}`)
            : `${revealedCells.size}/${totalCells} celle rivelate`}
        </div>
      </div>
      ...
      <div style={{ width:'100%', display:'flex', flexDirection:'column', gap:4, marginBottom:6 }}>
        {Array.from({length:MAX_ATTEMPTS}, (_,i) => {
          ...
          const gGrid = g ? FLAG_GRID[g.code] : null
          // Celle comuni con target
          const commonCells = g && !isCorrect ? getRevealedCells(country.code, g.code).size : null

          return (
            <div key={i} style={{ display:'flex', alignItems:'center', gap:6 }}>
              <div style={{ width:14, fontSize:10, color:C.textMuted, textAlign:'right', flexShrink:0 }}>{i+1}</div>

              {/* Miniatura bandiera tentativo */}
              {g ? (
              ...
              {g && !isCorrect && (
                <div style={{ flexShrink:0, minWidth:52, textAlign:'right' }}>
                  <span style={{ fontSize:11, fontWeight:700, color: commonCells>20?C.accent:C.textSecondary }}>
                    +{commonCells} celle
                  </span>
                </div>
              )}
      ...
      {tab==='howto' && (
        <div style={{ maxWidth:500, margin:'0 auto', width:'100%', padding:'20px 16px' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:12, fontSize:14, color:C.textSecondary, lineHeight:1.7 }}>
            {[
              ['🎨 Obiettivo', `Ricostruisci la bandiera nascosta indovinando i paesi! Ogni giorno c'è una nuova bandiera. Hai ${MAX_ATTEMPTS} tentativi.`],
              ['🔍 Come funziona', 'La bandiera è divisa in una griglia di celle. Ogni tentativo rivela le celle della bandiera da indovinare che hanno gli stessi colori del paese che hai scritto. Più colori in comune, più celle si colorano!'],
              ['🧩 Strategia', 'Inizia con paesi che hanno molti colori (come il Brasile o il Sudafrica) per rivelare più celle possibili. Usa i colori rivelati per capire la struttura della bandiera.'],
              ...
              ['☀️ Tema', 'Usa il pulsante sole/luna per alternare tra tema chiaro e scuro.'],
            ].map(([title, text]) => (
